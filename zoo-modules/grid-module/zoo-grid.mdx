...
## CSS variables
- `--grid-column-sizes` - specify value for `grid-template-columns`.      
...
- headers: `<zoo-grid-header slot="headercell"></zoo-grid-header>`;
  - attribute `sortable`
	  - `optional`
	  - `type` - `truthy` or `falsy`
  - attribute `sortableproperty`
	  - `optional`
	  - `type` - `text`
- rows: `<div slot="row"></div>` - arbitrary content;
- paginator: `<div slot="paginator"></div>`;
  - `optional`
  - `default` - `<zoo-grid-paginator><slot name="pagesizeselector"></slot></zoo-grid-paginator>`
- page size selector: `<div slot="pagesizeselector"></div>`;
  - `optional`, `mandatory` when `paginator` slot is defined;

## Events
### sortChange
`(sortChange)` - output event, whenever user changes sorting the following object will be dispatched on `zoo-grid`:      
```
{
	detail: {
...
### pageChange
- `(pageChange)` - output event, whenever user changes current page the following object will be dispatched on `zoo-grid`:
```
{
	detail: {
