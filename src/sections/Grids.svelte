...
<div class="grids-holder" bind:this="{gridHolder}">
	...
	<zoo-grid bind:this={zooGrid} stickyheader paginator currentpage="5" maxpages="20" 
		...
		{#each data as row} 
			<div class="example-row" slot="row">
				<div style="width: 30px;">
					<zoo-checkbox>
						<input checked="{row.activated}" slot="checkboxelement" type="checkbox"/>
					</zoo-checkbox>
				</div>
	...
	<h3>Grid which is always loading.</h3>

	<zoo-grid loading={true} paginator>
		{#each headers as header}
			<div slot="headercell" sortable={header.sortable ? 'sortable' : null} sortableproperty='{header.sortProperty}'>{header.title}</div>
		{/each}
		<div slot="norecords">
...
<script>
	...
	let data = [
		{activated: true, createdDate: today, status: 'READY', minWeight: '1 kg', maxWeight: '10 kg', deliveryDate: '', noOfPieces: 5, price: '12 EUR'},
		{activated: true, createdDate: today, status: 'DELIVERED', minWeight: '1 kg', maxWeight: '10 kg', deliveryDate: today, noOfPieces: 5, price: '12 EUR'},
		{activated: true, createdDate: today, status: 'READY', minWeight: '1 kg', maxWeight: '10 kg', deliveryDate: '', noOfPieces: 5, price: '12 EUR'},
		{activated: true, createdDate: today, status: 'DELIVERED', minWeight: '1 kg', maxWeight: '10 kg', deliveryDate: today, noOfPieces: 5, price: '12 EUR'},
		{activated: true, createdDate: today, status: 'READY', minWeight: '1 kg', maxWeight: '10 kg', deliveryDate: '', noOfPieces: 5, price: '12 EUR'}
	];
